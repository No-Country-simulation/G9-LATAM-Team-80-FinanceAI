...
export function BudgetsPage({ workspace }: PageProps) {
  ...
  async function guardarPresupuesto(event: React.FormEvent) {
    event.preventDefault();
    try {
      await workspace.agregarPresupuesto({ categoria, presupuesto: Number(presupuesto) });
      setError(''); setVista('listado');
      ...
      <div className="two-column-grid budget-top">
        <Card title="Resumen del presupuesto">
          <div className="budget-summary">
            ...
            <p>Gastado <b className="negative">{formatCurrency(-gastadoTotal)}</b> ({formatPercent(presupuestoTotal ? (gastadoTotal / presupuestoTotal) * 100 : 0)})</p>
            <div><span style={{ width: `${presupuestoTotal ? (gastadoTotal / presupuestoTotal) * 100 : 0}%` }} /></div>
