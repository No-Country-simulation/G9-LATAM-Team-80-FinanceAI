...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  return <main className="login-page">
    <section className="login-hero">
      ...
      <div><h1>Toma el control de tus finanzas</h1><p>Mejora tu salud financiera con análisis inteligentes, presupuestos personalizados y recomendaciones para alcanzar tus metas.</p></div>
      <ul className="login-benefits">
        <li><span>01</span><strong>Análisis inteligente</strong><small>Conoce tu situación financiera en tiempo real.</small></li>
        <li><span>02</span><strong>Datos persistentes</strong><small>Tus movimientos e historial se guardan de forma segura.</small></li>
        <li><span>03</span><strong>Mayor seguridad</strong><small>Acceso mediante sesión y contraseña cifrada.</small></li>
      </ul>
    </section>
    <form className="login-card" onSubmit={submit}>
      <div className="login-card-brand"><ChartBar size={30} weight="fill" /><strong>Salud Financiera</strong></div>
      <h2>Inicia sesión</h2><p>Usa la cuenta local incluida para probar el MVP</p>
      ...
      <button className="primary-button wide" type="submit" disabled={cargando}>{cargando ? 'Ingresando...' : 'Iniciar sesión'} <ArrowRight size={20} /></button>
    </form>
  </main>;
}
