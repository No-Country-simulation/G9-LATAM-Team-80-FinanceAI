...
```mermaid
flowchart TD
    subgraph FUENTES["Fuentes · en el repositorio"]
        DS[dataset_gastos.csv<br/>1.089 transacciones · 12 categorías]
        DIC[palabras-clave.json<br/>orden de prioridad]
    end
    subgraph NB["Notebook clasificador_gastos.ipynb"]
        EDA[EDA] --> V1[Clasificador v1<br/>reglas] --> V2[Modelo v2<br/>torneo → regresión logística] --> EV[Evaluación<br/>test + CV + benchmark]
    end
    DS --> EDA
    DIC --> V1
    EV --> PKL[modelo_clasificador.pkl<br/>+ modelo_info.txt]
    PKL -.->|joblib.load · predict| API[API del equipo<br/>texto crudo → 12 etiquetas]
```

**Modelo v3:** regresión logística sobre TF-IDF (1–2 gramas), con la normalización de texto dentro del pipeline. Métricas: 98.2 % en el conjunto de prueba (218 transacciones nunca vistas) · 96.0 % en validación cruzada de 5 pliegues (líder del torneo de 4 candidatos) · línea base por reglas: 81.9 %.

**Integración en dos líneas:** cargar `modelo_clasificador.pkl` una sola vez con `joblib.load` y clasificar con `modelo.predict(lista_de_descripciones)` — con el texto tal cual llegue. Salida: etiquetas técnicas de la lista cerrada de 12. Detalle y decisiones: [`docs/arquitectura.md`](docs/arquitectura.md) · [Notebook con métricas](clasificador_gastos.ipynb)

## 📋 Categorías oficiales

Lista **cerrada** (contrato v1.3): estas 12 etiquetas técnicas son los únicos valores válidos en datos, modelo y API.

| Etiqueta técnica (datos y API) | Nombre para mostrar (interfaz) |
|---|---|
...
| `cuidado_personal` | Cuidado personal y ropa |
