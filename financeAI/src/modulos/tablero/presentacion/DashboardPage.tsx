...
import { evolucionMensual } from '../../../compartido/constantes/datosDemo';
...
export function DashboardPage({ workspace, navegar }: PageProps) {
  ...
  return (
    <section className="page-stack">
      <PageHeader title="Hola, Juan" subtitle="Aqui tienes un resumen de tu salud financiera." />
      <div className="metric-grid">
        <MetricCard icon={<Wallet size={30} />} title="Ingreso mensual" value={formatCurrency(4500)} tone="green" />
        <MetricCard icon={<TrendDown size={30} />} title="Gasto total" value={formatCurrency(-gastoTotal)} tone="red" delta="8%" />
        <MetricCard icon={<PiggyBank size={30} />} title="Ahorro del mes" value={formatCurrency(600)} tone="blue" delta="15%" />
        <MetricCard icon={<TrendUp size={30} />} title="Nivel de endeudamiento" value="25%" tone="orange" delta="5%" />
      </div>
  ...
  return (
    <article className="metric-card">
      ...
      <div>
        ...
        <p>{delta ? `${delta} vs. mes anterior` : 'vs. mes anterior'}</p>
      </div>
    </article>
  );
}
      ...
      <div className="legend-list">
        {rows.map(([categoria, valor]) => (
          <div key={categoria}>
            ...
            <small>{formatPercent((valor / total) * 100)}</small>
          </div>
        ))}
      </div>
    </div>
...
function LineChart() {
  const max = Math.max(...evolucionMensual.map((item) => item.gastos));
  return (
